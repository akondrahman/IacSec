                     ...
                     if (token_type.eql? 'NAME') || (token_type.eql? 'VARIABLE')
                        ...
                        if (token_line==nxt_nxt_line)
                           ...
                           if ((((token_valu.include? "pwd") || (token_valu.include? "password") || (token_valu.include? "pass") ||
                                ...
                                (token_valu.include? "dsa") || (token_valu.include? "user")) && (! token_valu.include? "::")
...
PuppetLint.new_check(:no_hardcode_secret_uname) do
  def check
           invalid_kw_list = ['(', 'undef', 'true', 'false', 'hiera', 'secret', 'union',
                              '$', '{', 'regsubst', 'hiera_hash', 'pick', 'get_ssl_property',
                             'inline_template', 'under', 'mysql_password', '/', 'ssl_ciphersuite', '::']
           tokens.each do |indi_token|
               ...
               if (!nxt_token.nil?) && (!indi_token.nil?)
                  ...
                  nxt_nxt_token =  nxt_token.next_code_token # get the next next token to get key value pair
                  # removed these: (token_valu.include? "uuid") || (token_valu.include? "id") ||
...
PuppetLint.new_check(:no_hardcode_secret_password) do
    def check
           invalid_kw_list = ['(', 'undef', 'true', 'false', 'hiera', 'secret', 'union',
                              '$', '{', 'regsubst', 'hiera_hash', 'pick', 'get_ssl_property',
                             'inline_template', 'under', 'mysql_password', '/', 'ssl_ciphersuite', '::']
                       ...
                       if (token_type.eql? 'NAME') || (token_type.eql? 'VARIABLE')
                          if (token_line==nxt_nxt_line)
                             ...
                             if (((token_valu.include? "pwd") || (token_valu.include? "password") || (token_valu.include? "pass")
                                   ...
                                   print ' '
                                   notify :warning, {
                                      message: 'SECURITY:::HARD_CODED_SECRET_PASSWORD:::Do not hard code secrets. This may help an attacker to attack the system. You can use hiera to avoid this issue.@'+token_valu+'='+nxt_nxt_val+'@',
                                      line:    indi_token.line,
                                      column:  indi_token.column,
                                      token:   token_valu
                                   }
...
PuppetLint.new_check(:no_hardcode_secret_key) do
     def check
           invalid_kw_list = ['(', 'undef', 'true', 'false', 'hiera', 'secret', 'union',
                              '$', '{', 'regsubst', 'hiera_hash', 'pick', 'get_ssl_property',
                             'inline_template', 'under', 'mysql_password', '/', 'ssl_ciphersuite', '::']
                           ...
                           if (token_line==nxt_nxt_line)
                              ...
                              if ((((token_valu.include? "key") || (token_valu.include? "crypt") ||
                                   ...
                                   (token_valu.include? "dsa")) &&  (! token_valu.include? "::")
                                    ...
                                    notify :warning, {
                                       message: 'SECURITY:::HARD_CODED_SECRET_KEYS:::Do not hard code secrets. This may help an attacker to attack the system. You can use hiera to avoid this issue.@'+token_valu+'='+nxt_nxt_val+'@',
                                       line:    indi_token.line,
                                       column:  indi_token.column,
                                       token:   token_valu
                                    }
