       ...
       for file_ in files_:
           ...
           if(os.path.exists(full_p_file)):
             if (('playbook' in full_p_file) or ('role' in full_p_file) or ('inventor' in full_p_file) ) and ( full_p_file.endswith('.yml')  or full_p_file.endswith('.yaml')):
               ansi_.append(full_p_file)
             else:
               non_ansi.append(full_p_file)    

    tot = len(ansi_) + len(non_ansi) 
    return tot, len(ansi_)
...
def getRepoFilterStats(dir_par, type_par):
    ...
    for repo_ in repo_dirs:
        ...
        if (os.path.exists(full_dir_p)):
            if type_par == 'CHEF':
               tot_cnt, che_cnt = getChefCount(full_dir_p)  
            else:
               tot_cnt, che_cnt = getAnsibleCount(full_dir_p)                 
...
if __name__=='__main__':
   ...
   # the_dir = '/Users/akond/SECU_REPOS/ostk-ansi/'
   # getRepoFilterStats(the_dir, 'CHEF')
   # getRepoFilterStats(the_dir, 'ANSIBLE')
